...
def receber_resposta_servidor(cliente_soc, break_loop=False):
    while True:
        msg, addr = cliente_soc.recvfrom(1024)
...
def converter_e_enviar(cliente_soc, conteudo):
    cliente_soc.sendto(bytearray(gerar_hash(conteudo), 'utf-8'), (host, porta))
...
if __name__ == '__main__':
    try:
        clienteSoc = socket(AF_INET, SOCK_DGRAM)

        custom_print("---------------------------\n")
        custom_print("Bem vindo ao Terminal Chat!\n")
        custom_print("---------------------------\n")

        converter_e_enviar(clienteSoc, "BEM-VINDO")
